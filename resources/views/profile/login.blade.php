					...
					<div class="registration-steps-page-container">
							...
							<div class="registration-social-login-container">
								<div class="form-group">
									...
									<input type="text" class="form-control" id="School_name" placeholder="Enter School Name" list="fileSearch1" autocomplete="off" required>
		...
		$(document).on('click', '.regsubmit', function(){
			...
			var confirmpassword = $('#confirm-password').val();
              ...
              $.ajax({
                url: "{{ url('/adduser') }}",
                method: 'POST',
                data: { "_token" : "{{csrf_token()}}", 'schoolid': schoolid, 'role':role, 'name':name, 'email':email, 'password':password, 'confirmpassword':confirmpassword},
                success: function(data){
                  console.log(data)
                  if(data == 'error'){
